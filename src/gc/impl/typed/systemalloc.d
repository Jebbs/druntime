...
//used when calculating how many new pages to allocate (reduces number of divides)
enum PAGE_DIVISOR = 10*PAGE_SIZE;
...
struct AllocSystem
{
    static
    {
        ...
        //information about the memory in the heap
        size_t usedMemory, freeMemory, reservedMemory;
...
void* halloc(size_t size) nothrow
{
        ...
        AllocSystem.currentChunk.start + AllocSystem.currentChunk.chunkSize)
    {
        ...
        //mark this chunk as full and move onto the next
        auto fill = AllocSystem.currentChunk.chunkSize -
        ...
        //may throw out of memory error if there isn't enough memory
    ...
    AllocSystem.usedMemory+= size;
    AllocSystem.freeMemory-= size;
